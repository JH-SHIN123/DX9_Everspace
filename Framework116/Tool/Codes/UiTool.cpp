...
void CUiTool::OnBnClickedLoadButton()
{
	if (m_pListResource->size())
	{
		...
		for (auto& pDst : *m_pListResource)
			{
				CTextureList.AddString(pDst->wstrPrototypeTag);
				m_ListTexturePrototypeTag.emplace_back(pDst->wstrPrototypeTag);
...
void CUiTool::OnBnClickedCreateUiButton()
{
	if (0 <= CTextureList.GetCurSel())
	{
		...
		if (FAILED(m_pManageMent->Add_GameObject_InLayer_Tool(EResourceType::Static,
			L"GameObject_UI"
			, L"Layer_Ui",
			m_iUiCount,&UiDesc)))
		{
...
void CUiTool::OnLbnSelchangeCloneUIList()
{
	...
	{
		static_cast<CToolUI*>(*iter)->Set_UiTool(this);
...
void CUiTool::OnBnClickedLoadFileButton()
{
	...
	if (!fin.fail())
	{
		...
		while (true)
		{
			...
			if (FAILED(m_pManageMent->Add_GameObject_InLayer_Tool(EResourceType::Static, L"GameObject_UI"
